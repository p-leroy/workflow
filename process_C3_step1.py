...
os.makedirs(dir_tiles_1_2_5_6, exist_ok=True)  # tiles classified after lasclassify (classes 1, 2, 5 and 6)
...
work.run(f'lasclassify -i {tiles_g} -cores {cores} -odir {dir_tiles_1_2_5_6} -odix c -olaz')
# keep only ground points (class = 2)
work.run(f'las2las -i {tiles_g} -keep_class 2 -cores {cores} -odir {dir_tiles_2} -odix _ground -olaz')
# thin data
